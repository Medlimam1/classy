...
export async function POST(request: NextRequest) {
  try {
    ...
    const sadadAdapter = getPaymentAdapter(PaymentProvider.SADAD)
    const paymentIntent = await sadadAdapter.createPayment(
      amount,
      currency,
      { orderId, ...metadata }
    )
...
export async function GET(request: NextRequest) {
  try {
    ...
    const sadadAdapter = getPaymentAdapter(PaymentProvider.SADAD)
    const paymentIntent = await sadadAdapter.getPayment(paymentId)
