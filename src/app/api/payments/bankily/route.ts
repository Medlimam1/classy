...
export async function POST(request: NextRequest) {
  try {
    ...
    const bankilyAdapter = getPaymentAdapter(PaymentProvider.BANKILY)
    const paymentIntent = await bankilyAdapter.createPayment(
      amount,
      currency,
      { orderId, ...metadata }
    )
...
export async function GET(request: NextRequest) {
  try {
    ...
    const bankilyAdapter = getPaymentAdapter(PaymentProvider.BANKILY)
    const paymentIntent = await bankilyAdapter.getPayment(paymentId)
